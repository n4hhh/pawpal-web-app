...
-- Useful indexes
-- Compatibility block: if remote pulled schema already created older tables with
-- different column names, add missing columns so subsequent indexes/policies succeed.
DO $$
BEGIN
  IF NOT EXISTS (
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema='public' AND table_name='feed_posts' AND column_name='profile_id'
  ) THEN
    ALTER TABLE public.feed_posts ADD COLUMN profile_id uuid;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema='public' AND table_name='feed_posts' AND column_name='pet_id'
  ) THEN
    ALTER TABLE public.feed_posts ADD COLUMN pet_id uuid;
  END IF;
END
$$;
